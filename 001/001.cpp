...
void filling_population(vector<solution>& q, bool f)
{
	...
	while ((int)q.size() <= POPULATION_SIZE)
	{
		...
		if (rate < MUTATION_RATE || f)
		{
			...
			if (random < 0.5)
				tmp.first = hsm(tmp.first), tmp.second = hmm(tmp.second);
			else
				tmp.first = hmm(tmp.first), tmp.second = hsm(tmp.second);
		}
		...
		// If they're valid
		if (tmp.first.tour_length < 999999.0 && s.find(hashh(tmp.first.tour)) == s.end())
			q.push_back(tmp.first), s.insert(hashh(tmp.first.tour));
		if (tmp.second.tour_length < 999999.0 && s.find(hashh(tmp.second.tour)) == s.end())
...
solutionFull AddStation(solution& orig_sol)
{
	...
	*/
	...
	solutionFull res;
	for (int i = 0; i <= orig_sol.tour.size() - 2; i++)
	{
		if (orig_sol.tour[i] == orig_sol.tour[i + 1] && orig_sol.tour[i] == 0)
		{
			orig_sol.tour_length = 99999999.0;
			res.tour_length = orig_sol.tour_length;
			return res;
		}
	}
	res.id = orig_sol.id;
